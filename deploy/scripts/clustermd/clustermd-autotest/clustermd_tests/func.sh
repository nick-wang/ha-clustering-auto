...
check_ssh()
{
	NODE1="$(grep '^IP_NODE1' $CLUSTER_CONF | cut -d'=' -f2)"
	NODE2="$(grep '^IP_NODE2' $CLUSTER_CONF | cut -d'=' -f2)"
	[ -z "$NODE1" -o -z "$NODE2" ] && {
...
fetch_devlist()
{
	ISCSI_ID="$(grep '^SHARED_TARGET_LUN' $CLUSTER_CONF | cut -d'=' -f2)"
...
check_env()
{
	...
	check_ssh
	commands=(mdadm iscsiadm modinfo dlm_controld
		  udevadm crm crm_mon pgrep sbd)
	for ip in $NODE1 $NODE2
	do
		for cmd in ${commands[@]}
		do
